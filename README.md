...
## 🔧 SaaS Chatwoot Webhook Асуудал Шийдэх

### Асуудал: Хэрэглэгчийн текст ирэхгүй байна

Хэрэв хэрэглэгчийн бичсэн текст Chatwoot дээр ирэхгүй байгаа бол:

#### 1. Debug Webhook тохируулах

```
http://your-domain.com/webhook-debug
```

#### 2. Webhook Events шалгах

Chatwoot Settings → Integrations → Webhooks дээр:

- ✅ `message_created` идэвхжүүлэх
- ✅ `conversation_created` идэвхжүүлэх
- ❌ `message_updated` хаах
- ❌ `conversation_updated` хаах

#### 3. Debug заавар харах

```
http://your-domain.com/test-webhook
```

#### 4. Console логуудыг шалгах

```bash
# Серверийн логуудыг харах
tail -f your_app.log

# Python console дээр webhook data харах
```

#### 5. SaaS Chatwoot онцлог format

SaaS Chatwoot зарим тохиолдолд өөр webhook format ашигладаг:
...
```json
{
  ...
  "payload": {
    "message": {
      "content": "хэрэглэгчийн текст"
    }
  }
}
```

Код нь автоматаар олон төрлийн format-ыг шалгадаг.
